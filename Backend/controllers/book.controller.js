  ...
  Book.findOne({ _id: req.params.id })
    ...
    .then((book) => {
      ...
      const existingRating = book.ratings.find(
        (r) => String(r.userId) === req.auth.userId
      );
      if (existingRating) {
...
exports.updateBook = (req, res, next) => {
  ...
  Book.findOne({ _id: bookId })
    .then((book) => {
      ...
      const isSame = Object.keys(bookObject).every(
        (key) => book[key] === bookObject[key]
      );

      // Check if image needs to be updated
      const isImageUpdated =
        req.file && req.file.filename !== book.imageUrl.split("/").pop();

      if (isSame && !isImageUpdated) {
        return res
          .status(200)
          .json({ message: "Pas de changements détectés!" });
      }

      const oldImageFilename = book.imageUrl
